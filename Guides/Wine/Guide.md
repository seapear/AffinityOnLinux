...
# 🧭 Add Affinity to your Application Menu / Toolbar

Once Affinity runs successfully from the terminal, you can make it appear in your desktop’s launcher, App Grid, or Dock just like any other program.

### 1. Create the Launcher File
Open a terminal and run:
```bash
nano ~/.local/share/applications/affinity.desktop
```

Paste the following contents (update the path to `Affinity.exe` if you installed Photo 2/Designer 2/Publisher 2):

Note: *Replace the "$USER" with your actual username as .desktops don't like $USER.* 
```ini
[Desktop Entry]
Name=Affinity
Comment=Affinity V3 by Canva via Wine
Exec=env WINEPREFIX="/home/$USER/.affinity" wine "/home/$USER/.affinity/drive_c/Program Files/Affinity/Affinity/Affinity.exe"
Type=Application
StartupNotify=true
Icon=/home/$USER/.local/share/icons/affinity.svg
Categories=Graphics;Photography;Design;
Path=/home/$USER/.affinity/drive_c/Program Files/Affinity/Affinity
```
...
### 2. Install the Custom Icon
Download the SVG and place it where desktops expect to find icons:

```bash
mkdir -p ~/.local/share/icons
curl -L -o ~/.local/share/icons/affinity.svg https://raw.githubusercontent.com/seapear/AffinityOnLinux/main/Assets/Icons/Affinity-Canva-Squircle.svg
```

GNOME and most modern environments will scale SVG icons perfectly.
...
### 4. Refresh Application Database
```bash
